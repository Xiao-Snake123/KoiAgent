...
class KoiLive:
    def __init__(self, cookies_str, bot: KoiReplyBot, order_handler=None, context_manager=None):
        ...
        # 健康检查文件（供容器 HEALTHCHECK 判断进程存活与连接状态）
        self.health_path = os.getenv("HEALTH_FILE", "data/health.json")

        # 幂等去重：防止重连 / 重复推送导致重复回复
    ...
    async def handle_message(self, message_data, websocket):
        ...
        try:
            ...
            if self.dedup.seen(dedup_key):
                logger.info(f"重复消息已跳过 (会话: {chat_id}, 发送者: {send_user_id})")
                return
            ...
            if send_user_id == self.myid:
                ...
                self.bot.record_message(chat_id, "assistant", send_message)
            ...
            if self.is_manual_mode(chat_id):
                ...
                self.bot.record_message(chat_id, "user", send_message)
                return
            # 检查是否为带中括号的系统消息
            ...
            if not item_info:
                ...
                api_result = self.api.get_item_info(item_id)
